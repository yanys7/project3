...

{
    ...
    my_samples.resize( 1000, 3 * q + 5, 0.0 );
    my_labels.resize( 1000, n_mixtures, 0.0 );
    my_probs.resize( 1000, n_mixtures * q, 0.0 );
  ...
  void Residual( size_t it )
  {
   ...
   { 
     ...
     //printf( "r_jk %d prop0 %.2E prop1 %.2E prop2 %.2E prop3 %.2E \n", 
     //    r_jk[ 0 ], prop[ 0 ], prop[ 1 ], prop[ 2 ], prop[ 3 ] ); fflush( stdout );
    ...
    //     r_jk[ 0 ], prop[ 0 ], prop[ 1 ], prop[ 2 ], prop[ 3 ] ); fflush( stdout );
       ...
       for ( size_t k = 0; k < n_mixtures; k ++ ) 
       {
         ...
         MultiVariableNormal<T> my_mvn( temp, Sigma_temp[ k ] );
         Sigma_Mixture[ k ] = my_mvn.Inverse();
         mu_mixture[ k ] = Sigma_Mixture[ k ] * w;
         Sigma_det[ k ] = my_mvn.LogDeterminant();
     ...
     //printf( "r_jk %d prop0 %.2E prop1 %.2E prop2 %.2E prop3 %.2E max_prop %.2E \n", 
     //    r_jk[ 0 ], prop[ 0 ], prop[ 1 ], prop[ 2 ], prop[ 3 ], max_prop ); fflush( stdout );
     

       std::discrete_distribution<int> dist_r ( { prop[ 0 ], prop[ 1 ], prop[ 2 ], prop[ 3 ] } );       
       //std::random_device rd;
       //std::mt19937 gen(rd());
       //std::discrete_distribution<size_t> dist_r( { 10, 40, 40, 10 } );
       r_jk[ j ] = dist_r( generator );
       MultiVariableNormal<T> my_mvn( mu_mixture[ r_jk[ j ] ], Sigma_Mixture[ r_jk[ j ] ] );
       Data<T> bvn_sample = my_mvn.SampleFrom( 1 );
       beta_m[ j ] = bvn_sample[ 0 ];
       alpha_a[ j ] = bvn_sample[ 1 ];
       //printf( "beta_m %.4E alpha_a %.4E \n", beta_m[ j ], alpha_a[ j ] ); fflush( stdout );
       //printf( "updated2 r_jk %d prop0 %.2E prop1 %.2E prop2 %.2E prop3 %.2E \n", 
       //    r_jk[ j ], prop[ 0 ], prop[ 1 ], prop[ 2 ], prop[ 3 ] ); fflush( stdout );
     ...
     //printf( "r_jk0 %d v_11 %.2E v_10 %.2E v_01 %.2E v_00 %.2E \n", r_jk[ 0 ], v_11, v_10, v_01, v_00 ); fflush( stdout );
     }
     ...
     for ( size_t k = 0; k < n_mixtures; k ++ ) 
     {
       ...
       MultiVariableNormal<T> my_mvn( temp, Vk_temp );
       Vk_inv[ k ] = my_mvn.Inverse();
       values1[ k ] = Vk_temp(0, 0);
       values2[ k ] = Vk_temp(1, 1);
	...
	//printf( "v_11 %.2E v_10 %.2E v_01 %.2E v_00 %.2E \n", my_labels( count, 0 ), my_labels( count, 1 ), my_labels( count, 2 ), my_labels( count, 3 ) ); fflush( stdout );
        count += 1;
	...
	string my_labels_filename = string( "labels_" ) + to_string( (int)q1 ) + to_string( (int)q2 ) + string( ".txt" );
        my_labels.WriteFile( my_labels_filename.data() );
