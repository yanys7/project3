...
class Variables
{
  ...
  void Residual( size_t it )
  {
   ...
   void Iteration( size_t burnIn, size_t it )
   {
     ...
     for ( size_t j = 0; j < q; j ++ )
     {
       sigma_m1[ j ] = Z1[ j ] * Var[ 0 ];
       sigma_ma1[ j ] = Z2[ j ] * Var[ 1 ];
     }
     ...
     for ( size_t j = 0; j < q; j ++ )
     {
      upsi.rand( 0.0, Z1[ j ] / ( 1.0 + Z1[ j ] ) );
      T tempps = beta_m[ j ] * beta_m[ j ] / ( 2 * Var[ 0 ] );
      T ub = ( 1 - upsi[ 0 ] ) / upsi[ 0 ];
      T Fub = 1 - std::exp( - tempps * ub );
      if ( Fub < 1E-04 ) Fub = 1E-04;

      up.rand( 0.0, Fub );
      T eta = - std::log( 1 - up[ 0 ] ) / tempps;
      Z1[ j ] = 1.0 / eta;

      upsi.rand( 0.0, Z2[ j ] / ( 1.0 + Z2[ j ] ) );
      tempps = alpha_a[ j ] * alpha_a[ j ] / ( 2 * Var[ 1 ] );
      ub = ( 1 - upsi[ 0 ] ) / upsi[ 0 ];
      Fub = 1 - std::exp( - tempps * ub );
      if ( Fub < 1E-04 ) Fub = 1E-04;

      up.rand( 0.0, Fub );
      eta = - std::log( 1 - up[ 0 ] ) / tempps;
      Z2[ j ] = 1.0 / eta;
     ...
     /** update Var */
     if ( false ) {
     hmlp::Data<T> tempt( 2, 1 );
     tempt.resize( 2, 1, 0.0 );
     for ( size_t j = 0; j < q; j ++ ) {
       tempt[ 0 ] += beta_m[ j ] * beta_m[ j ] / ( 2 * Z1[ j ] );
       tempt[ 1 ] += alpha_a[ j ] * alpha_a[ j ] / ( 2 * Z2[ j ] );
      }

     for ( size_t i = 0; i < 2; i ++ ) {
       hmlp::Data<T> utau( 1, 1 );
       utau.rand( 0.0, Var[ i ] / ( 1.0 + Var[ i ] ) );
       T ubt = ( 1.0 - utau[ 0 ] ) / utau[ 0 ];
       T Fubt = gsl_cdf_gamma_P( ubt, ( q + 1 ) / 2.0, 1.0 / tempt[ i ] );
       printf( "cdf_gamma_P %.3E \n", Fubt ); fflush( stdout );
       Fubt = std::max( Fubt, 1E-06 );
       hmlp::Data<T> ut( 1, 1 );
       ut.rand( 0.0, Fubt );
       Var[ i ] = 1.0 / gsl_cdf_gamma_Pinv( ut[ 0 ], ( q + 1 ) / 2.0, 1.0 / tempt[ i ] );
       printf( "ubt %.3E Fubt %.3E ut %.3E \n", ubt, Fubt, ut[ 0 ] ); fflush( stdout );
       printf( "cdf_gamma_Pinv %.3E \n", Var[ i ] ); fflush( stdout );
     }
