  ...
  void Residual( size_t it )
  {
   ...
   { 
     ...
     for ( size_t j = 0; j < q; j ++ )
     {
       ...
       for ( size_t k = 0; k < n_mixtures-1; k ++ ) 
       {
       ...
       Sigma_det[ 1 ] = my_mvn4.LogDeterminant();
       ...
       Sigma_det[ 2 ] = my_mvn5.LogDeterminant();
       ...
       beta_m[ j ] = mu_mixture[ r_jk[ j ] ][ 0 ];
       alpha_a[ j ] = mu_mixture[ r_jk[ j ] ][ 1 ];
       ...
       if ( r_jk[ j ] == 1 ) {
          //Data<T> mu7( 1, 1, mu_mixture[ r_jk[ j ] ][ 0 ]);
      	  //Data<T> sigma7( 1, 1, Sigma_Mixture[ r_jk[ j ] ]( 0, 0 ) );
          //MultiVariableNormal<T> my_mvn7( mu7, sigma7 );
          //Data<T> bvn_sample = my_mvn7.SampleFrom( 1 );
       ...
       if ( r_jk[ j ] == 2 ) {
          //Data<T> mu8( 1, 1, mu_mixture[ r_jk[ j ] ][ 1 ] );
          //Data<T> sigma8( 1, 1, Sigma_Mixture[ r_jk[ j ] ]( 1, 1 ) );
          //MultiVariableNormal<T> my_mvn8( mu8, sigma8 );
          //Data<T> bvn_sample = my_mvn8.SampleFrom( 1 );
          beta_m[ j ] = 0;
          //alpha_a[ j ] = bvn_sample[ 0 ];
