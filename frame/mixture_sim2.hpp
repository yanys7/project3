...

{
    ...
    Z_k.resize( 1, n_mixtures, 1.0 );
    std::gamma_distribution<T> dist_Zk0( a, 1.0 / b );
    for ( size_t i = 0; i < n_mixtures; i++ ) {
      Z_k[ i ] = 1.0 / dist_Zk0( generator );
    }
   ...
   { 
     ...
     for ( size_t j = 0; j < q; j ++ )
     {
       ...
       for ( size_t k = 0; k < n_mixtures; k ++ ) 
       {
         ...
         for ( size_t i = 0; i < Sigma_temp[ k ].dim() * Sigma_temp[ k ].dim(); i++ ) {
           Sigma_temp[ k ][ i ] /= Z_k[ k ]; 
         }
       }
       ...
       for ( size_t k = 0; k < 1; k ++ ) 
       {
         ...
         prop[ k ] += 0.5 * ( mu_mixture[ k ][ 0 ] * w[ 0 ] + mu_mixture[ k ][ 1 ] * w[ 1 ] ) + 0.5 * ( - Sigma_det[ k ] + Vk_det[ k ] - std::log( Z_k[ k ] * Z_k[ k ] ) );
       }
       ...
       prop[ 1 ] += 0.5 * ( mu_mixture[ 1 ][ 0 ] * w[ 0 ] ) + 0.5 * ( - Sigma_det[ 1 ] + Vk_det[ 1 ] - std::log( Z_k[ 1 ] ) );
       ...
       prop[ 2 ] += 0.5 * ( mu_mixture[ 2 ][ 1 ] * w[ 1 ] ) + 0.5 * ( - Sigma_det[ 2 ] + Vk_det[ 2 ] - std::log( Z_k[ 2 ] ) );
	  ...
	  std::normal_distribution<T> dist_norm_m0( mu_mixture[ 1 ][ 0 ], std::sqrt( Sigma_Mixture[ 1 ]( 0, 0 ) ) );
       ...
       Wishart_m[ r_jk[ j ] ]( 0, 0 ) += beta_m[ j ] * beta_m[ j ] / Z_k[ r_jk[ j ] ];
       Wishart_m[ r_jk[ j ] ]( 0, 1 ) += beta_m[ j ] * alpha_a[ j ] / Z_k[ r_jk[ j ] ];
       Wishart_m[ r_jk[ j ] ]( 1, 0 ) += beta_m[ j ] * alpha_a[ j ] / Z_k[ r_jk[ j ] ];
       Wishart_m[ r_jk[ j ] ]( 1, 1 ) += alpha_a[ j ] * alpha_a[ j ] / Z_k[ r_jk[ j ] ];
     ...
     /** update Z_k */
     hmlp::Data<T> param( n_mixtures, 1, 0.0 );
     hmlp::Data<T> temp1( d, 1, 0.0 );
     hmlp::Data<T> temp2( 1, d, 0.0 );
     for ( size_t i = 0; i < q; i++ ) {
       if ( r_jk[ i ] != 3 ) {
       temp1[ 0 ] = beta_m[ i ]; temp1[ 1 ] = alpha_a[ i ];
       temp2[ 0 ] = beta_m[ i ]; temp2[ 1 ] = alpha_a[ i ]; 
       param[ r_jk[ i ] ] += 0.5 * ( temp2 * Vk_inv[ r_jk[ i ] ] * temp1 )[ 0 ]; 
       }   
     }

     for ( size_t i = 0; i < n_mixtures-1; i++ ) {
       std::gamma_distribution<T> dist_Z( r_count[ i ] + a, 1.0 / ( b + param[ i ] ) );
       Z_k[ i ] = 1.0 / dist_Z( generator );
     }
