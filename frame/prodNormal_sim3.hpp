...
class Variables
{
  ...
  void Residual( size_t it )
  {
   ...
   void Iteration( size_t burnIn, size_t it )
   {
     ...
     for ( size_t j = 0; j < q; j ++ )
     {
       sigma_m1[ j ] = 1.0 / ( 1.0 + Z2[ j ] ) * Var[ 0 ];
       sigma_ma1[ j ] = Z2[ j ] / ( 1.0 + Z2[ j ] ) * Var[ 1 ];
     }
     ...
     for ( size_t j = 0; j < q; j ++ )
     {
       ...
       //if ( beta_m[ j ] < 1E-07 ) beta_m[ j ] = 1E-07;
       //if ( alpha_a[ j ] < 1E-07 ) alpha_a[ j ] = 1E-07;
       T bj = std::abs( beta_m[ j ]*alpha_a[ j ] ) / std::sqrt( Var[ 0 ]*Var[ 1 ] );
       T tj = Z2[ j ] * std::abs( alpha_a[ j ] ) * std::sqrt( Var[ 0 ] ) / ( std::abs( beta_m[ j ] ) * std::sqrt( Var[ 1 ] ) );
       //Z2[ j ] = ( std::sqrt( 1 + bj*bj ) - 1 ) * Var[ 1 ] / ( alpha_a[ j ] * alpha_a[ j ] );

       upsi.rand( 0.0, std::sqrt( tj ) );
       T ub = upsi[ 0 ] * upsi[ 0 ];
       if ( ub < 1E-05 ) ub = 1E-05;
       //printf( "beta_m %.3E alpha_a %.3E Z2 %.3E \n", beta_m[ j ], alpha_a[ j ], Z2[ j ] ); fflush( stdout );
       //printf( "Iter %4lu tj %.3E bj %.3E \n", it, tj, bj ); fflush( stdout );

       using boost::math::inverse_gaussian;
       inverse_gaussian ig( 1.0, bj );
       T Fub = cdf( ig, ub );
       //printf( "Iter %4lu ub %.3E Fub %.3E \n", it, ub, Fub ); fflush( stdout );

       up.rand( Fub, 1.0 );
       if ( up[ 0 ] == 1.0 ) { up[ 0 ] = 0.999; }
       tj = quantile( ig, up[ 0 ] );
       //printf( "Iter %4lu up %.3E \n", it, up[ 0 ] ); fflush( stdout );

       if ( isnan(tj) || tj > 1E07 ) tj = 1E07;
       Z2[ j ] = tj * ( std::abs( beta_m[ j ] ) * std::sqrt( Var[ 1 ] ) ) / ( std::abs( alpha_a[ j ] ) * std::sqrt( Var[ 0 ] ) );
       //printf( "Iter %4lu tj %.3E Z2 %.3E \n", j, tj, Z2[ j ] ); fflush( stdout );
      ...
      if ( it > burnIn && it % 10 == 0 )
      {
        ...
        my_samples( count, 3* (int)q + 2 ) = Var[ 1 ];
